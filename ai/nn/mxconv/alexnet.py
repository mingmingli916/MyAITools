...
def AlexNet(classes):
    ...
    # Block #1: first CONV => POOL layer set
    conv1_1 = mx.sym.Convolution(data=data, kernel=(11, 11), stride=(4, 4), num_filter=96)
    act1_1 = mx.sym.LeakyReLU(data=conv1_1, act_type='elu')  # elu is better
    bn1_1 = mx.sym.BatchNorm(data=act1_1)  # the channel axis is determined by mxnet automatically
    pool1 = mx.sym.Pooling(data=bn1_1, pool_type='max', kernel=(3, 3), stride=(2, 2))
    do1 = mx.sym.Dropout(data=pool1, p=.25)

    # Block #2: second CONV => POOL layer set
    ...
    do2 = mx.sym.Dropout(data=pool2, p=.25)
    ...
    pool3 = mx.sym.Pooling(data=bn3_3, kernel=(3, 3), stride=(2, 2), pool_type='max')
    do3 = mx.sym.Dropout(data=pool3, p=.25)

    # Block #4: first FC layers
    flatten = mx.sym.Flatten(data=do3)
    fc1 = mx.sym.FullyConnected(data=flatten, num_hidden=4096)
    act4_1 = mx.sym.LeakyReLU(fc1, act_type='elu')
    bn4_1 = mx.sym.BatchNorm(data=act4_1)
    do4 = mx.sym.Dropout(data=bn4_1, p=.5)  # not .25

    # Block #5: second FC layer
    ...
    do5 = mx.sym.Dropout(data=bn5_1, p=.5)

    # softmax classifier
    fc3 = mx.sym.FullyConnected(data=do5, num_hidden=classes)
    model = mx.sym.Softmax(data=fc3, name='softmax')
